...
function [movietrial]=decode_pres_movie_encode_immov(smrfile, itmfile, moviesStart, moviesEnd);
...
cueon=find(hiblob(1:end-3)==14 & hiblob(2:end-2)==36 & hiblob(3:end-1)==03 & hiblob(4:end)==36); %find all marker 35s which indicate the cue before the movie has turned on
...
for i=1:length(cueon);  %scroll through all the cue on times
    ...
    if hiblob(cueon(i)+1)==36;  %if the next marker is 36 (cue off) then...
        movietrial(i).cueoff=marktimes(cueon(i)+2);%...record the cue off time in the structure
    end
    if hiblob(cueon(i)+3)>256*255;  %if the next value after cue off is greater than FF*256 then this is the movie number
        movietrial(i).moviecnd=hiblob(cueon(i)+3)-256*255;  %calculate the movie cnd number and record it in the structure
        movietrial(i).movieon=marktimes(cueon(i)+3);    %record the time that the move cam on in the structure
    end
    endmov=find(hiblob(cueon(i)+4:end)==35,1,'first')+cueon(i)+5-2;
    if ~isempty(endmov);
    movietrial(i).framenum=hiblob(cueon(i)+4:endmov)-1000;  %then all the remaining markers are frame numbers; record them
    movietrial(i).frametime=marktimes(cueon(i)+4:endmov);   %record the time that each frame was presented.
    else
    movietrial(i).framenum=hiblob(cueon(i)+4:end)-1000;  %then all the remaining markers are frame numbers; record them
    movietrial(i).frametime=marktimes(cueon(i)+4:end);   %record the time that each frame was presented.
