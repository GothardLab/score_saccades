...
eyeY = xy(2,:)*cal.param(2,2)+cal.param(1,2);
    ...
    eyeX = eyeData(1,:)*cal.param(2,1)+cal.param(1,1);
    eyeY = eyeData(2,:)*cal.param(2,2)+cal.param(1,2);
end;
...
for i=1:length(trialinfo); %scroll through the movie trials
    if strcmp(trialinfo(i).trialtype,'good');
        ...
        trialinfo(i).eyexImOn=eyeX(imon:imon+3000);  %cut teh eye data from one frame to the next and record it in the movietrial structure
        trialinfo(i).eyeyImOn=eyeY(imon:imon+3000);  %cut teh eye data from one frame to the next and record it in the movietrial structure
    end
end
save([calibPath,eyeFile(1:end-4),'_calibrated_image_eye.mat']);
