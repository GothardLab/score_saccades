...
%function [movietrial]=decode_pres_movie_encode_with_fixspots(smrfile, itmfile, moviesStart, moviesEnd);
...
moviesStart = moviesStartS;
moviesEnd = moviesEndS ;
...
fixon=find(hiblob(1:end-1)==35 & hiblob(2:end)~=36); %find all marker 35s which indicate the cue before the movie has turned on
for i=1:length(cueon);  %scroll through all the cue on times
    ...
    nextfixInd=(find(fixon>cueon(i),1,'first'));
       
    if ~isempty(nextfixInd); %if this is not the last movie to be presented for the day then...
       nextfix=fixon(nextfixInd);
        movietrial(i).framenum=hiblob(cueon(i)+3:nextfix-1)-1000;    %then take all the markers before the next cue appears and calculate teh frame numbers
        movietrial(i).frametime=marktimes(cueon(i)+3:nextfix-1); %record the times that each frame came on
    else
